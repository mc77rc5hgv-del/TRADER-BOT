...
Два независимых элемента интерфейса, поверх обычных inline-кнопок под
конкретными сообщениями:

- **Chat menu button** (иконка слева от строки ввода) — прямой запуск Mini
  App в один тап, настраивается один раз при старте бота
  (`bot.set_chat_menu_button(MenuButtonWebApp(...))` в `app/bot/main.py`,
  без `MINI_APP_URL` откатывается на стандартный `MenuButtonCommands`).
- **Persistent reply keyboard** (`app/bot/keyboards.py::main_reply_keyboard()`)
  — 5 кнопок навигации (AI-анализ, Сетапы, Алерты, Точность AI, Подписка)
  закреплены под строкой ввода (`is_persistent=True`) и не пропадают при
  прокрутке чата, в отличие от inline-кнопок под одним сообщением. Кнопка
  отправляется один раз — после онбординга и по `/app` — и остаётся видимой
  до конца сессии. Каждая кнопка сопоставлена точным текстом
  (`F.text == BTN_...`) и вызывает ту же функцию рендера, что и старые
  inline callback-хендлеры (`CB_...`), которые оставлены только ради
  обратной совместимости с уже отправленными сообщениями.
